{pkgs, ...}: let
  kscreenScript = pkgs.mkShellScriptBin "kscreen-id.py" ''
    ...
    ksd_path = "${pkgs.kdePackages.libkscreen}/bin/kscreen-doctor"
    ...
    def run_kscreen_doctor():
        ...
        if cmd_exists("kscreen-doctor"):
            result = subprocess.run(
                [ksd_path, "-o"], capture_output=True, text=True
            )
