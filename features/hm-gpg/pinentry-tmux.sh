...
if [[ -n "${PINENTRY_TMUX_CALLER:-}" ]]; then
	...
	# Forward the popup pane's actual $TERM instead of hardcoding "xterm" --
	# hardcoding it discards whatever 256-color/truecolor terminfo the
	# real theme depends on. Fall back to a sane default only if $TERM
	# itself isn't a terminfo entry pinentry's bundled ncurses recognizes.
	popup_ttytype="${TERM:-xterm-256color}"
	if ! infocmp "$popup_ttytype" >/dev/null 2>&1; then
		popup_ttytype="xterm-256color"
	fi

	"${PINENTRY_TMUX_PROGRAM}" \
		--ttyname="${popup_tty}" \
		--ttytype="${popup_ttytype}" \
		--lc-ctype="${LC_CTYPE:-c}"
	exit $?
fi
...
# any failure inside this subshell (e.g. a bad flag, a failed capture)
# fires the outer script's own `cleanup`/`abort` traps prematurely,
# deleting the fifos out from under the foreground process that still
# needs them.
({
	trap - EXIT INT USR1
	...
	# Create the popup.
	#
	# Deliberately NOT passing -s/-S here: those set popup-style/
	# popup-border-style for just this invocation, overriding whatever
	# theme is already configured via `set -g popup-style ...` /
	# `set -g popup-border-style ...` in tmux.conf. Omitting them lets
	# the user's own configured theme apply instead.
